...
export function OrderForm({ isOpen, onClose, selectedPackage }: OrderFormProps) {
  ...
  useEffect(() => {
    const savedContent = localStorage.getItem("YourExitPlans-content")
    if (savedContent) {
      ...
      <div className="bg-[#0f0f0f] glass-border-enhanced rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden">
              ...
              <div key={step} className="flex items-center">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium transition-colors ${
                    step <= currentStep ? "bg-[#3B82F6] text-black" : "bg-neutral-700 text-neutral-400"
                  }`}
                >
                ...
                {step < 4 && (
                  <div
                    className={`w-8 h-0.5 mx-2 transition-colors ${
                      step < currentStep ? "bg-[#3B82F6]" : "bg-neutral-700"
                    }`}
                  />
                )}
              </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="text-right">
                      <div className="text-2xl font-bold text-[#3B82F6]">{order.package?.price}</div>
                    </div>
                  </div>
                </CardContent>
              </Card>
                    ...
                    <Button
                      ...
                      className={`h-16 ${
                        order.has3DModel === true
                          ? "bg-[#3B82F6] text-black hover:bg-[#3B82F6]/90"
                  ...
                  <Card
                    ...
                    className={`cursor-pointer transition-all hover:scale-[1.02] ${
                      order.modelingAddOn?.name === option.name
                        ? "glass-border-enhanced bg-[#3B82F6]/10"
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <span className="text-[#3B82F6] font-semibold">+₹{option.price_inr.toLocaleString()}</span>
                          </div>
                          <p className="text-sm text-neutral-400">{option.complexity}</p>
                        </div>
                        {order.modelingAddOn?.name === option.name && <Check className="h-5 w-5 text-[#3B82F6]" />}
                      </div>
                    </CardContent>
                  </Card>
                ))}
                        ...
                        <CardContent className="p-4">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="text-right">
                              ...
                              {order.renderPackage?.name === option.name && (
                                <Check className="h-4 w-4 text-[#3B82F6] ml-2 inline" />
                              )}
                            </div>
                          </div>
                        </CardContent>
            ...
            <div className="space-y-6">
              ...
              <Card className="glass-border-subtle border-neutral-800 bg-neutral-900/50">
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="flex justify-between items-center py-3 bg-[#3B82F6]/10 rounded-lg px-4">
                    <h4 className="text-lg font-bold text-white">Total</h4>
                    <span className="text-2xl font-bold text-[#3B82F6]">{formatPrice(calculateTotal())}</span>
                  </div>
                </CardContent>
              </Card>
            </div>
        ...
        <div className="flex items-center justify-between p-6 border-t border-neutral-800">
          ...
          {currentStep < 4 ? (
            ...
            <Button onClick={handleConfirmOrder} className="bg-[#3B82F6] text-black hover:bg-[#3B82F6]/90">
